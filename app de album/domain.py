...
def gravar_dados(album, ano, artista, lancamento):
    ...
    bd.gravar_dados(album, ano, artista, lancamento)


# Aqui é chamada a função para preencher a TreeView do banco de dados no gui.py armazenando os valores recebidos pelo
# bd.py em uma matriz chamada lista e a retornando para o gui.py.
def preencher_bd():

    lista = bd.preenche_bd()
    return lista
...
def preencher_artista(artista):
    ...
    lista = bd.preenche_artista(artista)
    return lista
